...
- **Paste history (粘贴历史)** — Menu (or optional global hotkey, **default off**) opens a picker of today’s unique `copy` / `copy_paste` rows; ⌘F focuses the search field, Enter searches all historical log kinds; selecting a row pastes via clipboard + ⌘V into the previously focused app (**Accessibility** required for paste into other apps). Programmatic inject is not re-logged.
