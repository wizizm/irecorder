...
- **打字**：**分支 A（访达/备忘录/企微等）**：AX 差分；中文输入法下暂存拉丁，按输入框里**完整拉丁词**判断（保留 `test`/`zhong`，丢弃拼音碎片）；中文输入法激活时不会因停顿把未确认拼音冲进日志。**分支 B（Cursor / VS Code 系）**：忽略不可靠 Monaco 文本框；中文输入法下关闭按键回退，聊天正文在**发送后**采集（Enter / ⌘Enter）。分支 B 规则**不会**套用到访达等原生应用。
- **复制 / 粘贴**：监听剪贴板与全局 ⌘V；短时间内「复制后立刻粘贴同一段」合并为 `copy_paste`
- **粘贴历史**：菜单（或可选全局快捷键，**默认关闭**）打开面板，默认列出今日去重后的 `copy` / `copy_paste`；⌘F 聚焦顶部搜索框，回车搜索全部历史日志类型；选中一行后通过剪贴板 + ⌘V 粘贴到先前焦点所在 App（向其他 App 粘贴需 **辅助功能** 权限）。程序化注入不会再记入日志。
- **按行缓冲**：停输 N 秒（默认 3，可调）写一行；Enter 不换行（中文上屏常用 Enter）
- **菜单栏**：暂停 / 继续、打开今日日志、粘贴历史、设置（目录、保留天数、截断、开机启动）、**检查更新…**（GitHub Release zip）、**帮助**（Issues）
- **隐私友好**：数据仅存本机；密码框内打字不记录

## 系统要求

- macOS 14+
- **辅助功能**权限（打字采集、全局快捷键，以及向其他 App 合成 ⌘V 粘贴）
...
2. 生成 `dist/iRecorder.app` 与 `dist/iRecorder.app.zip`（发布到 GitHub Release 时上传 zip，供「检查更新」使用）
...
| 粘贴历史快捷键 | 全局快捷键打开粘贴历史面板；**默认关闭**，需在设置中自行启用并录制；需辅助功能权限 |

## 限制

- 高度自绘 / 游戏仍可能记不到打字；Cursor 聊天中文在发送后记录；代码编辑器边打边记中文仍不支持
...
iRecorder **不收集、不上传**任何数据。日志只写在你指定的本地目录。辅助功能权限仅用于读取已上屏文本、监听全局快捷键，以及从粘贴历史向其他 App 合成 ⌘V 粘贴。
